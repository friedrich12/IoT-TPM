...
sgx_status_t ecall_start_tls_client(sgx_enclave_id_t eid, const char * inter, const char * priv, const char * iot_ip, const char * serverip, char * filedata)
{
	sgx_status_t				status;
	ms_ecall_start_tls_client_t ms;
	ms.ms_inter		   = inter;
	ms.ms_inter_len	   = inter ? strlen(inter) + 1 : 0;
	ms.ms_priv		   = priv;
	ms.ms_priv_len	   = priv ? strlen(priv) + 1 : 0;
	ms.ms_iot_ip	   = iot_ip;
	ms.ms_iot_ip_len   = iot_ip ? strlen(iot_ip) + 1 : 0;
	ms.ms_serverip	   = serverip;
	ms.ms_serverip_len = serverip ? strlen(serverip) + 1 : 0;
	ms.ms_filedata	   = filedata;
	ms.ms_filedata_len = filedata ? strlen(filedata) + 1 : 0;
